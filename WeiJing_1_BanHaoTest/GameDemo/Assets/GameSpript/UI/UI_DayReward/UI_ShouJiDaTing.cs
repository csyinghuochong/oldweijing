...
public class UI_ShouJiDaTing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Obj_ShowParentSet.GetComponent<RectTransform>().sizeDelta = new Vector2(1366, 3600);
        ...
        for (int i = 0; i <= honorStoreItemList.Length - 1; i++) {
            ...
            if (honorStoreItemList[i] != "")
            {
                ...
                {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_1.GetComponent<Text>().color = new Color(0.41f, 1.0f, 0.41f);
                    rewardPro_1 = rewardXingNum_1 + "颗:" + rewardPro_1 + "(已激活)";
                }
                else {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_1.GetComponent<Text>().color = new Color(0.56f, 0.56f, 0.56f);
                    rewardPro_1 = rewardXingNum_1 + "颗:"  + rewardPro_1 + "(未激活)";
                }

                if (nowXingNum >= rewardXingNum_2)
                {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_2.GetComponent<Text>().color = new Color(0.41f, 1.0f, 0.41f);
                    rewardPro_2 = rewardXingNum_2 + "颗:" + rewardPro_2 + "(已激活)";
                }
                else
                {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_2.GetComponent<Text>().color = new Color(0.56f, 0.56f, 0.56f);
                    rewardPro_2 = rewardXingNum_2 + "颗:"  + rewardPro_2 + "(未激活)";
                }

                if (nowXingNum >= rewardXingNum_3)
                {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_3.GetComponent<Text>().color = new Color(0.41f, 1.0f, 0.41f);
                    rewardPro_3 = rewardXingNum_3 + "颗:"  + rewardPro_3 + "(已激活)";
                }
                else
                {
                    duiHuanTitleObj.GetComponent<UI_ShouJiTitleObj>().Obj_Pro_3.GetComponent<Text>().color = new Color(0.56f, 0.56f, 0.56f);
                    rewardPro_3 = rewardXingNum_3 + "颗:"  + rewardPro_3 + "(未激活)";
                }
                    ...
                    if (showNum <= 7)
                    {
                        ...
                        if (showNum == 7)
                        {
                            ...
                            if (nextID != Game_PublicClassVar.Get_function_DataSet.DataSet_ReadData("NextID", "ID", nextID, "ShouJiItem_Template")) {
                                showTitle_Y = showTitle_Y - 180.0f; //增加标题高度
                            }
                        }
                    }
