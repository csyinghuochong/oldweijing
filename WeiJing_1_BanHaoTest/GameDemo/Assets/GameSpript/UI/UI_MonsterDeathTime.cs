...
public class UI_MonsterDeathTime : MonoBehaviour {
    ...
    public float DeathRebirthTimeSum;
    private float deathTimeSum;
    public string deathMonsterName;
    //离线时间
    public float OffLineTime;
    private float offLineTimeSum;
	...
	void Start () {
        Obj_DeathTime.GetComponent<Text>().text = "怪物重生时间：" + (int)DeathTime + "秒";
        Obj_OffLineTime.GetComponent<Text>().text = "怪物离线重生时间：" + (int)OffLineTime + "秒";
	...
	void Update () {
        ...
        if (deathTimeSum>=1.0f)
        {
            ...
            if ((int)DeathTime <= 0)
            {
                Obj_DeathTime.GetComponent<Text>().text = "怪物将立即刷新";
                Obj_OffLineTime.GetComponent<Text>().text = "怪物将立即刷新";
            }
            else {
                //展示在线时间
                ...
                switch (shouType) { 
                    case "0":
                        showStr = "怪物重生时间：" + remainTime + "秒";
                        break;

                    case "1":
                        showStr = "怪物重生时间：" + minuteTime + "分" + remainTime + "秒";
                        break;

                    case "2":
                        showStr = "怪物重生时间：" + hourTime + "时" + minuteTime + "分" + remainTime + "秒";
                        break;
                ...
                {
                    case "0":
                        offLine_showStr = "怪物离线重生时间：" + offLine_remainTime + "秒";
                        break;

                    case "1":
                        offLine_showStr = "怪物离线重生时间：" + offLine_minuteTime + "分" + offLine_remainTime + "秒";
                        break;

                    case "2":
                        offLine_showStr = "怪物离线重生时间：" + offLine_hourTime + "时" + offLine_minuteTime + "分" + offLine_remainTime + "秒";
                        break;
