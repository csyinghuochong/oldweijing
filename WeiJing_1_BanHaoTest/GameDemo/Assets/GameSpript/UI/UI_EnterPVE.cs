...
public class UI_EnterPVE : MonoBehaviour {
	...
	void Start () {
        ...
        if (SceneTransferID != "" || SceneTransferID != "0")
        {
            ...
            if (costTiLi == "0")
            {
                UI_CostTiLi.GetComponent<Text>().text = "无消耗";
            }
            else
            {
                UI_CostTiLi.GetComponent<Text>().text = "消耗体力：" + costTiLi;
            }
        }
	}
    ...
    public void EnterPVE() {
        ...
        if (int.Parse(ChapterSonID) >= 5001) {
            Game_PublicClassVar.Get_function_UI.GameGirdHint_Front("地图暂不开放,敬请期待！");
            return;
        }
        */
        //每次进入新地图设置血量为满的
