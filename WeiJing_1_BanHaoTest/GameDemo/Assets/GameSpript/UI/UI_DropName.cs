...
public class UI_DropName : MonoBehaviour
{
    ...
    public float HideDropPro;                   //极品装备掉落概率
	...
	private bool itemDropStatus;				//掉落道具掉落状态

    private bool DropMoveStatus;                //掉落的物体向上移动开启
    private bool DropMoveDownStatus;            //掉落的物体向下移动开启
    private int moveTime;                       //3D模型移动的次数
    private float rangeForward;
    ...
    private float DropUpTime;
    private bool DropUpStatus;
	...
	void Start () {
        ...
        //dropItemModel.transform.localRotation = 
	...
	void Update () {
        ...
        if (distance <= 30.0f)
        {
            ...
            if (DropNameUpdateStatus == true)
            {
                ...
                obj_LabDropName.transform.GetComponent<Text>().text = dropItemName;
                if (DropItemID == "1") {
                    obj_LabDropName.transform.GetComponent<Text>().text = DropItemNum.ToString() + dropItemName;
                }
                //显示道具品质颜色
                obj_LabDropName.GetComponent<Text>().color = Game_PublicClassVar.Get_function_UI.QualityReturnColor(dropItemQuality);
            }
            //修正掉落物体的位置
            if (obj_dropName != null) {
                obj_dropName.transform.localPosition = new Vector3(vec3_droName.x, vec3_droName.y, 0);
        ...
        if(DropItemID=="1"){
            ...
            if (rose_Status.RoseStatus == "2") {
                ...
                if (dis <= 0.5f)
                {
                    Game_PublicClassVar.Get_function_Rose.SendReward("1", DropItemNum.ToString());
                    IfRoseTake = true;
                }
            }
        }
